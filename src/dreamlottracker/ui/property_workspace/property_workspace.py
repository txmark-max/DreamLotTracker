...
class PropertyWorkspace(QDialog):
    def __init__(self, property_id: int, parent=None):
        ...
        tabs.addTab(self._price_history_tab(), "History")
    ...
    def _location_tab(self):
        ...
        for label, field in [
            ("Minutes to Gulf Shores", self.minutes_to_gulf_shores),
            ("Minutes to Foley", self.minutes_to_foley),
            ("Minutes to Fairhope", self.minutes_to_fairhope),
            ("Minutes to Pensacola", self.minutes_to_pensacola),
            ("Flood Zone", self.flood_zone),
            ("Wetlands", self.wetlands),
            ("Road Type", self.road_type),
            ("Paved Road", self.paved_road),
        ]:
            form.addRow(label, field)

        note = QLabel("Drive time is used for scoring. GPS coordinates remain on the Overview tab for future automatic lookups.")

...
class DashboardPage(QWidget):
    def __init__(self):
        ...
        recalculate = QPushButton("Recalculate Scores")
        recalculate.clicked.connect(self.recalculate_scores)
    ...
    def create_card(self, label: str) -> QLabel:
        card = QLabel(f"0\n{label}")
    ...
    def refresh(self):
        ...
        self.total_card.setText(f"{stats['total']}\nProperties")
        self.active_card.setText(f"{stats['active']}\nActive")
        self.dream_card.setText(f"{stats['dream_lots']}\nDream Lots")
        self.average_card.setText(f"{stats['average_score']:.1f}\nAverage Score")

    def recalculate_scores(self):
        count = self.service.recalculate_scores()
        self.refresh()

        QMessageBox.information(
            self,
            "Scores Recalculated",
            f"Recalculated scores for {count} properties.",
        )
